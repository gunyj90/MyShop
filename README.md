...
여러 상품을 한번에 주문할 수 있으며 주문 후, [주문금액, 배송비, 총 지불금액]을 확인할 수 있습니다.
<br/>
<br/>
## 실행방법
...
### 동시주문에 대한 재고관리
주문이 발생하면 요청한 상품수량 만큼 재고가 차감됩니다.
...
'stock = :재고'로 업데이트가 아닌 'stock = stock - :요청수량'으로 가감합니다.
stock이 요청수량보다 크거나 같은 경우 갱신합니다.
수정된 row의 개수가 없을 경우 SoldOutException을 발생 시킵니다.
이와 같은 방법으로 네트워크 통신량을 줄이고 성능저하를 일으키지 않으며 재고상태를 0이상으로 유지할 수 있습니다.
<br/>
<br/>
### 상품 마이그레이션
서비스 기동시 초기 상품정보를 세팅합니다.
제공된 파일타입에 따라 이를 읽고 Object로 변환합니다.
해당 프로젝트는 csv파일을 읽고 이를 상품데이터로 변환하여 H2의 상품테이블에 저장합니다.

* csv파일 외 추가 및 변경될 파일타입을 고려하여 interface와 구현체를 분리합니다. (DataReader <- CsvDataReader)
* 데이터는 csv->String->Item 순으로 변경되고 DataReader에서 String으로 변환합니다. Item변환은 FunctionalInterface인 Parseable을 caller로 부터 전달받아 실행 되어집니다.
* 마이그레이션 시, 배치처리(saveAll)를 통해 네트워크 왕복횟수를 줄입니다.
<br/>

### 조회성능 개선
상품 전체 조회시 dto로 변환하므로써 불필요한 캐싱을 방지합니다.
```code
List<ItemDto> findAllByStockGreaterThan(int stock);
```

하나의 주문에 다건의 상품과 수량을 요청할 수 있습니다. 이때 상품의 유효성을 체크하기 위해 상품정보를 조회해 옵니다. 요청 상품건수마다 데이터를 조회해 오는 것에 개선점이 있습니다.
* 요청데이터타입인 OrderItemRequest는 Map타입인 캐시를 관리합니다. 캐시는 상품ID를 키로 가지며 요청수량을 값으로 합니다.
* 상품 조회는 where 상품번호 in (...) 로 수정하여 한번에 다건을 조회합니다.
* 조회한 상품을 기준으로 OrderItemRequest에서 캐시를 통해 다시 요청수량을 맵핑합니다.
